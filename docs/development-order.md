...
- **账号系统重构**：邮箱从必填改为可选
  - 注册仅需用户名 + 密码，不再需要邮箱
  - 登录支持用户名/邮箱两种方式
  - 邮箱仅通过 OAuth（Google/GitHub）绑定获取，确保邮箱有效
  - 找回密码/邮件通知等邮箱服务仅对已绑定 OAuth 的用户可用
  - 纯密码用户忘记密码通过管理员重置
  - 相关文档：`docs/architecture.md` §8、§12.3
...
- 修复：render API 创建 ArticleWikiLink 记录（此前从未写入，导致链接状态始终为空）
- 修复：Token 页面 dark mode tick 字体颜色（改用 CSS 类选择器覆盖）
- 优化：文章管理页链接状态改为卡片标题旁圆点指示器（🟡需更新/🟢已同步），下拉菜单移除状态行
...
**邮箱设计**：邮箱为可选字段，仅通过 OAuth 绑定获取。
- 注册仅需用户名 + 密码，无需邮箱
- 登录支持用户名/邮箱两种方式
- 邮箱仅通过 OAuth（Google/GitHub）绑定获取，确保邮箱有效
- 找回密码/邮件通知等邮箱服务仅对已绑定 OAuth 的用户可用
- 纯密码用户忘记密码通过管理员重置
- Resend 仅用于：已绑定 OAuth 的用户主动触发密码找回/通知推送（退信风险极低）

### 模块 8.1：数据库与认证框架

| 任务 | 说明 |
|------|------|
| 8.1.1 | 新增 User、Account、Session、VerificationToken 模型（User.email 改为可选） |
...
| 任务 | 说明 |
|------|------|
| 8.2.1 | 注册页面（用户名 + 密码，无需邮箱） |
| 8.2.2 | 登录页面（用户名/邮箱 + 密码 + OAuth 按钮） |
| 8.2.3 | Credentials provider 支持用户名或邮箱登录 |
| 8.2.4 | 密码找回功能（仅限已绑定 OAuth 的用户，通过 OAuth 邮箱发送重置邮件） |
| 8.2.5 | OAuth 绑定页面（登录后在设置中关联 Google/GitHub，获取已验证邮箱） |
| 8.2.6 | 纯密码用户忘记密码 → 联系管理员重置 |
...
| 8.4.3 | 个人设置页面（修改密码、绑定/解绑 OAuth、查看已绑定的邮箱） |

### 验收标准

- [ ] 用户可注册（用户名 + 密码）
- [ ] 用户可登录（用户名/密码 / Google / GitHub）
- [ ] 用户可在设置中绑定 OAuth
- [ ] 仅已绑定 OAuth 的用户可使用密码找回
- [ ] 博主可登录并访问仪表盘
- [ ] 评论显示用户名
- [ ] 词条申请记录用户信息
...
- [x] 文章可隐藏/显示、可置顶
- [x] 权限模型统一落地（文章/词条/图片）
- [x] 文章管理页可查看链接状态、批量链接（卡片标题旁圆点指示同步状态，下拉菜单显示链接数+检测时间）
- [x] 自动链接 Worker 按配置周期运行
- [x] Token 用量持久化记录
- [x] Token 用量仪表盘可查看（今日/7天/本月/按类型/最近记录）
- [x] 超阈值自动告警
