...
**最后更新**：2026-06-30 (v0.13.0)

### v0.13.0 2026-06-30
- **账号系统重构**：邮箱从必填改为可选，仅通过 OAuth 绑定获取
  - §2.5：认证说明更新为 Credentials（用户名+密码）+ 邮箱仅通过 OAuth 获取
  - §10：阶段8 描述更新（移除邮箱验证，新增 OAuth 绑定）
  - §12.3：邮箱配置说明新增设计原则——仅用于已绑定 OAuth 用户
  - 注册仅需用户名 + 密码；找回密码仅对已绑定 OAuth 用户可用
...
- §6.4.7：translator2 输出新增 `contentSnapshot` 字段，供 translate 任务自行从 DB 读取增量基准（与 review 统一），不再依赖调用方传入 `oldSourceContent`
- §5.1：discovery 任务更新——增量模式，只扫描变化部分，复用上次发现的候选词条
...
|------|------|------|
| 邮件 | Resend SDK | 通知发送 |
| 认证 | NextAuth.js v5 | Credentials（用户名+密码）+ 可选 Google/GitHub OAuth，JWT session，Prisma 适配器。邮箱为可选字段，仅通过 OAuth 绑定获取，确保邮箱有效 |
...
| 类型 | 输入 | 输出 |
|------|------|------|
| review | articleId, 文章内容 | 审查报告（结构化 JSON），含 contentSnapshot + contentMap 供下次增量 |
| translate | articleId, sourceLanguage, targetLanguage, targetArticleId | 译文 Markdown + translations 映射 + translatedGroups + contentSnapshot |
| discover | articleId, articleSlug, articleLang | 候选词条列表 + contentSnapshot + existingCandidates 供下次增量 |
...
- `contentSnapshot` — 翻译后的 article body（无 frontmatter），供下次增量 diff 对比
- `translatedContent` — 完整译文内容（含 frontmatter，写文件使用）
 ...
 * @returns TranslateResult — 含 translations（下次增量复用）和 rawContent（contentSnapshot 来源）
 */
...
`TranslateResult` 包含以下关键字段供下次增量使用：
- `translations: TranslationMap` — 原文内容 → 译文的映射（逐行+多行 chunk 混合）
- `translatedContent: string` — 完整译文的拼接结果（含 frontmatter）
- `translatedGroups: ChangeGroupDetail[]` — 发送给 AI 的每个子块的区间信息，供详情页渲染

**重要**：translator2 不直接输出 `contentSnapshot`，而是通过 `translatedContent` 提供。Worker 层在保存 `AiTask.output` 时，额外存储 `contentSnapshot` 字段（从 `stripFrontmatter(result.translatedContent)` 提取），作为下次增量 diff 的旧内容基准。详见 worker.ts 的 `processTranslate`。
...
#### 6.4.11 discovery.ts 增量实现（词条发现场景）

词条发现使用与 review/translate 相同的 `detectChanges` + `splitRange` + `buildContext` 通用 pipeline，但复用策略不同：

| 任务 | 复用的"内容" | 复用依据 |
|------|------------|----------|
| review | ReviewChunk（审查报告段落） | contentMap（原文内容 → ReviewChunk） |
| translate | 译文文本 | TranslationMap（原文内容 → 译文） |
| discover | 候选词条列表 | existingCandidates（原文内容 → DiscoveryCandidate[]） |

**差异原因**：发现的结果是"候选词条列表"而非"内容替换"。未变化的行仍然需要保留其候选词条列表，但 merged 到最终结果中时会去重。具体来说：

1. **未变化范围**：从上次发现的 `existingCandidates` 中，按内容查找上次为对应行发现了哪些词条，合并到最终结果
2. **变化范围**：只对变化部分调用 AI 进行词条发现
3. **去重**：合并词条后做 case-insensitive 去重（与当前一致）
 ...
 * 只扫描变更部分，复用上次发现的候选词条。
 *
 * @param oldSourceContent - 旧版本内容（空字符串 = 全量）
 * @param newSourceContent - 新版本内容
 * @param existingCandidates - 上次发现的候选词条（原文内容 → DiscoveryCandidate[]）
 * @param articleLang - 文章语言
 * @param customDiscoveryPrompt - 可选自定义 prompt
 * @returns 去重后的候选词条列表 + contentSnapshot
 */
export async function incrementalDiscover(
  oldSourceContent: string,
  newSourceContent: string,
  existingCandidates: Record<string, DiscoveryCandidate[]>,
  articleLang: string,
...
Worker 中的 `processDiscover` 调用方式与 review 一致：
1. 读取上次 discovery 任务的 `output.contentSnapshot` 和 `output.existingCandidates`
2. 调用 `incrementalDiscover()` 传入这两个值
3. 保存结果到 AiTask.output（含新的 `contentSnapshot` 和 `existingCandidates`）
...
| 7 | AI 翻译功能 | 阶段 3、5 |
| 8 | 账号系统 + 评论系统（注册、登录、OAuth 绑定、密码管理、评论） | 阶段 1 |
| 9 | AI 词条发现与生成 | 阶段 4、5 |
...
> **设计原则**：邮箱为可选字段，仅通过 OAuth 绑定获取。Resend 仅用于已绑定 OAuth 的用户主动触发密码找回或通知推送，退信风险极低。
