    ...
    <div className="flex flex-col gap-6">
      ...
      {!preview && !refining && (
        <p className="text-xs text-muted-foreground">
          输入词条名称后点击&ldquo;AI
          分析&rdquo;，系统将自动分析词条类型、定义和重要性，确认后提交申请。
        </p>
      )}
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${typeColor(preview.type)}`}
              >
              ...
              <div className="relative size-8">
                <svg className="size-8 -rotate-90" viewBox="0 0 32 32">
                  ...
                  <circle
                    ...
                    strokeLinecap="round"
                    className={
                      preview.importance >= 0.9
                        ? "stroke-green-500"
                        : preview.importance >= 0.7
                          ? "stroke-blue-500"
                          : preview.importance >= 0.5
                            ? "stroke-yellow-500"
                            : "stroke-slate-400"
                    }
                  />
      ...
      {!preview ? (
        <div className="flex items-center gap-3 pt-2">
          ...
          <Button type="button" variant="outline" onClick={() => router.push("/admin/wiki")}>
            取消
          </Button>
        </div>
      ) : (
...
function EditForm({ initialData }: { initialData: WikiEntryInitialData }) {
  ...
  return (
    <form onSubmit={form.handleSubmit(onSave)} className="flex flex-col gap-6">
      ...
      {/* Tags */}
      <TagInput label="标签" value={tags} onChange={setTags} placeholder="输入标签后按回车添加" />
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-xs text-muted-foreground">简短的术语定义，用于 hover 预览。</p>
        <textarea
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-xs text-muted-foreground">AI 生成的内容，阶段 3 为手动预留。</p>
        <textarea
          {...form.register("ai")}
          rows={6}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-xs text-muted-foreground">参考来源列表。</p>
        <textarea
          {...form.register("ref")}
          rows={4}
          className="rounded-lg border border-input bg-background px-3 py-2 text-sm font-mono outline-none focus:border-primary resize-y"
